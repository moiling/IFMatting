...
The code can be used in two ways:
    1. By importing solve_foregound_background in your code:
        ```
            ...
            ...
            # For scribles input
            cfm = closed_form_matting.closed_form_matting_with_scribbles(image, scribbles)

            # For trimap input
            cfm = closed_form_matting.closed_form_matting_with_trimap(image, trimap)

            # For prior with confidence
            cfm = closed_form_matting.closed_form_matting_with_prior(
...
def compute_weight(img, mask=None, eps=10**(-7), win_rad=1):
    ...
    Args:
        ...
        eps: regularization parameter controlling cfm smoothness
    ...
    # L = np.array((nz_indsVal, (nz_indsRow, nz_indsCol)), shape=(h*w, h*w))
    L = scipy.sparse.csr_matrix((nz_indsVal, (nz_indsRow, nz_indsCol)), shape=(h*w, h*w))
    return L
...
def closed_form_matting_with_prior(image, prior, prior_confidence, consts_map=None):
    """Applies closed form matting with prior cfm map to image.

    Args:
        image: 3-dim numpy matrix with input image.
        prior: matrix of same width and height as input image holding apriori cfm map.
        prior_confidence: matrix of the same shape as prior hodling confidence of prior cfm.
        consts_map: binary mask of pixels that aren't expected to change due to high
            prior confidence.

    Returns: 2-dim matrix holding computed cfm map.
    """
...
def test(arg_image, arg_trimap, arg_output):

    image = cv2.imread(arg_image, cv2.IMREAD_COLOR) / 255.0

    trimap = cv2.imread(arg_trimap, cv2.IMREAD_GRAYSCALE) / 255.0
    alpha = closed_form_matting_with_trimap(image, trimap)

    output = alpha

    imsave(arg_output, output * 255.0)
...
if __name__ == "__main__":
    ...
    # main()
    test(input_file, tri_map_file, save_file)
